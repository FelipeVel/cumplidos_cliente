...
angular.module('contractualClienteApp')
  .controller('InformeGyCertificadoCCtrl', function (token_service, cookie, $sessionStorage, $scope, $http, $translate, uiGridConstants, cumplidosCrudRequest, $route, $q, documentoRequest, $window, $sce, gestorDocumentalMidRequest, $routeParams, utils, amazonAdministrativaRequest, nuxeoMidRequest, cumplidosMidRequest, titanMidRequest) {
    ...
    self.documento_contratista = token_service.getPayload().documento;
    ...
    // self.formatear_actividades_especificas = function () {
    //   self.actividades_especificas_disponibles = self.actividades_especificas.map(function (actividad_especifica) {
    //     return { actividad_especifica, "activo": true }
    //   })
    // }

    //self.formatear_actividades_especificas();
    ...
    //   "InformacionContrato": {
    //     "Supervisor": "",
    //     "ValorContrato": "24530202",
    //     "CPS": 1265,
    //     "fechaCPS": "16/06/2021",
    //     "Dependencia": "Oficina Asesora de Sistemas",
    //     "Sede": "Administrativa",
    //     "Objeto": "En virtud del presente contrato el contratista se compromete a prestar servicios de apoyo a la gestión como tecnólogo de manera autónoma e independiente como desarrollador de soluciones informáticas que soportan los procedimientos SIGUD para la optimización de Cumplidos CPS y VE, en cuanto a: ciclo de vida documental, novedades contractuales después de generar el cumplido, módulo de parametrizaciones, automatización del informe de gestión y certificado de cumplimiento, optimización de desempeño e interfaz e integración con el módulo de notificaciones, ciñéndose al modelo de gestión y evaluación de necesidades y requerimientos utilizado por la Oficina Asesora de Sistemas, en el marco de los planes, programas y proyectos para el plan de desarrollo vigente en la Universidad.",
    ...
    //       "Nombre": "JUAN CAMILO MARTINEZ LOPEZ",
      ...
      cumplidosMidRequest.get('informe/' + self.contrato + '/' + self.vigencia + '/' + self.mes + '/' + self.anio).then(function (response) {
        ...
        if (response.status == 200) {
          if (response.data.Data == null) {
            ...
            self.Informe.DocumentoContratista = token_service.getPayload().documento;
        ...
        content: [
          ...
          },
          {
            ...
            table: {
              ...
              heights: 40,
              body: [
                [{ text: 'OBJECTO DEL CONTRATO:', bold: true, fontSize: 11, fillColor: '#CCCCCC', margin: [0, 15, 0, 0] }, { text: self.informacion_informe.Objeto, alignment: 'justify', fontSize: 10 }]
              ]
            }
          },
          {
